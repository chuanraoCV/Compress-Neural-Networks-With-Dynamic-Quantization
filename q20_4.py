...
def dynamic_quantizantion(v):
  ...
  n0 = tf.stop_gradient(tf.floor(tf.log(v_max)/tf.log(tf.constant(2,tf.float32)))) ###  +1
  ...
  mask8 = tf.where( (v_abs <= (3/2.0)*n8), tf.ones(shape) * n8, mask7)
  with G.gradient_override_map({"Sign": "Identity"}):
    mask_result = tf.sign(v)*tf.stop_gradient(mask8)
...
if not tf.gfile.Exists('q4_20model'):
    tf.gfile.MakeDirs('q4_20model')
saver.save(sess,'./q4_20model/res.ckpt')
